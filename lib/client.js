...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function percentColor(percent) {
      // 随已用额度 0→100 由绿 → 黄 → 红 渐变（HSL 色相 120→0）
      var p = percent == null ? 0 : Math.max(0, Math.min(100, percent));
      var hue = 120 - (p / 100) * 120;
    ...
    function render(card) {
      ...
      if (state.error !== null && state.data === null) {
        ...
        for (var i = 0; i < WINDOW_DEFS.length; i++) {
          ...
          fill.style.width = "0%";
          fill.style.background = (w && w.available) ? percentColor(w.percent) : "rgba(128,128,128,.35)";
    ...
    async function fetchQuota() {
      ...
      state.loading = true;
      // 不在请求开始/期间整卡重绘（避免 loading 状态造成的闪烁），
      // 只在拿到数据后统一渲染一次。
      try {
    ...
    function mountCard() {
      ...
      // 本地时钟 tick：仅在“分钟”变化时局部替换峰谷条节点（不整卡重绘，避免每次刷新抖动）。
      var dsLastMin = null;
      var tickTimer = setInterval(function () {
        try {
          var min = beijingDayMin();
          if (dsLastMin === null) { dsLastMin = min; return; }
          if (min === dsLastMin) return;
          dsLastMin = min;
          var b = card.querySelector(".ocg-body");
          var old = b && b.querySelector(".ocg-ds");
          var fresh = buildDsSection();
          if (old && fresh && old.parentNode && old !== fresh) old.replaceWith(fresh);
        } catch (e) { /* noop */ }
      }, 30000);
      fetchQuota();
